...
class DevicePulseTimeline extends LitElement {
    ...
    render() {
        ...
        let events = ! this._filterByDeviceId ?
            this._events :
            this._events.filter(ev => ev.device_id == this._filterByDeviceId)

        let orientation = this._forceVertical ?
            'vertical' : this._config.orientation;
                    ...
                    <div class=${classMap({
                        ...
                        <div class="timeline-content shift-${shiftLeft}">
                            ${when(
                                events.length === 0,
                                () => this._initialized ? 
                                        html`<div class="timeline-empty-state"> No Events occurred in the last ${this._config.hours_back} hours </div>` :
                                        html``,
                                () => repeat(
                                    events,
...
class DevicePulseTimelineEditor extends LitElement {
    ...
    _valueChanged(evt) {
        ...
        // Force device name clip if horizontal orientation
        if (config.orientation === 'horizontal') {
            config = {...config, device_name_clip: true}
            // If responsive is disabled, remove breakpoint data
    ...
    _computeLabel(schema) {
        switch (schema.name) {
            ...
            case "hours_back": return "Hours Back";
            case "orientation": return "Orientation Mode";
            case "device_name_clip": return "Clip Device Name";
            case "responsive_orientation": return "Responsive Orientation";
            case "responsive_breakpoint": return "Responsive Breakpoint";
        }
        return undefined;
    }
    ...
    _computeHelper(schema) {
        switch (schema.name) {
            case "device_name_clip": return "Truncate device names that are too long instead of wrapping them";
            case "responsive_orientation": return "Automatically revert to vertical orientation if there is no enough space";
        }
        return undefined;
    }
        ...
        const schema = [
            ...
            }},
            {
                ...
                schema: [
                    ...
                    ...(isVertical ?
                        [{name: "device_name_clip", required: true, selector: {
                            boolean: {}
                        }}] :
                        [
                            ...
                            ...(hasResponsiveBreakpoint ?
                                [{name: "responsive_breakpoint", required: true, selector: {
                                    number: {
                                        min: 280,
                                        max: 1024,
                                        step: 1,
